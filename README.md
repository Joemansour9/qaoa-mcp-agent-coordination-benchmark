...
| `results_I10_n16_simulate.json`, `results_I11_n16_simulate.json` | Already `seed_simulator=42`-current. Source for $\mathcal{R}$(I10), $\mathcal{R}$(I11) |
| `results_n16_simulate_seeded_2026-08-05.json` | Seeded I1–I5 simulation baseline, used to compute $\mathcal{R}$ in Table `tab:utility` |
...
| File | Backs |
|---|---|
| `multi_indicator_analysis_2026-08-05.json` | $\mathcal{R}$ and $\Delta\bar{r}$ for I1–I5, all three summary-statistic definitions. Source for Table `tab:utility`. |
| `i1_i6_i11_analysis.py` / `.json` | The extended-predictor analysis for I1, I7, I8, I10, I11 ($n=5$): $\mathcal{R}$, $\Delta\bar r$ (three definitions), and exact-permutation Spearman correlations, computed directly from the `results/` files above with nothing hardcoded. Source for Table `tab:extended_predictor` and reproduces its printed values exactly ($r_s=-0.300/+0.100/-0.100$, exact $p=0.683/0.950/0.950$). |
