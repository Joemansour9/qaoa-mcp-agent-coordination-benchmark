...
| File | Backs |
|---|---|
| `multi_indicator_analysis.py` / `_2026-08-05.json` | R, Sim/HW p=2, and delta-r-bar for I1–I5, all three summary-statistic definitions, via both exact-permutation and asymptotic Spearman correlations (plus Pearson cross-check). Reads directly from `results_n16_simulate_seeded_2026-08-05.json` (sim) and `results_I1_I4_rerun_2026-08-05.json` / `results_repeated_optionB.json` (I1–I4 / I5 hardware), nothing hardcoded. Source for Table `tab:utility` and every R/delta-r-bar number quoted in Section 6.2.1's prose; reproduces all of them exactly (mean: r_s=-0.600, exact p=0.350, asymptotic p=0.285; median: r_s=0.000, exact p=1.000; best-of-3: r_s=+0.700, exact p=0.233, asymptotic p=0.188). Replaces an earlier, unscripted version of this JSON that stored only the asymptotic p-value with no saved generating code. |
