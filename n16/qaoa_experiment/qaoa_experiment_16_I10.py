"""
qaoa_experiment_16_I10.py
=========================
QAOA Benchmarking — Instance I10 (two competing hubs)
...
I10 alone — a single instance measured from a genuinely different task
...
Usage:
  ...
  python qaoa_experiment_16_I10.py --mode simulate --p_max 4 \\
      --matrix mcp_agent_data_16_I10_cost_matrix.csv

  # Phase 2 (costs QPU time) — hardware run once warm-start params are ready.
  python qaoa_experiment_16_I10.py --mode hardware --p_max 2 \\
      --hw_depths 2 --matrix mcp_agent_data_16_I10_cost_matrix.csv \\
      --sim_results results_I10_n16_simulate.json
"""
...
def load_matrix(csv_path='mcp_agent_data_16_I10_cost_matrix.csv'):
    if not os.path.exists(csv_path):
        raise FileNotFoundError(
            f"Cost matrix not found at '{csv_path}'.\n"
            f"Run mcp_data_generator_16_I10.py first.")
...
def build_instances(W):
    """
    I10 is a single, standalone instance — measured directly from a
    genuinely different task mix (two competing dominant hub pairs,
    see mcp_data_generator_16_I10.py), not derived from I1 by
    scaling/permutation the way I2-I5 were. Kept as a single-element
    list so run_experiment()'s loop structure is unchanged.
    """
    return [(W.copy(), 'I10_two_hubs')]
...
def run_experiment(mode='simulate', ibm_token=None,
                   ...
                   matrix_path='mcp_agent_data_16_I10_cost_matrix.csv',
                   max_iter=200, hw_depths=None, sim_results_path=None,
                   hardware_name='ibm_marrakesh'):

    W_full    = load_matrix(matrix_path)
    instances = build_instances(W_full)   # single-element list for I10
    ...
    if mode == 'hardware':
        ...
        preloaded_sim = preloaded[0]['qaoa_sim']   # single I10 instance
        for p in hw_depths:
    ...
    if mode in ('hardware', 'both'):
        ...
        # For I10 to be comparable, it MUST run on the same device — NOT
        ...
        try:
            ...
            raise RuntimeError(
                ...
                f"I10 must run on the same device as I1-I5 ({hardware_name}) "
...
def print_summary(results, p_values):
    print(f"\n{'='*60}")
    print(f"FINAL SUMMARY — I10 (two competing hubs)")
    print(f"{'='*60}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='QAOA n=16 MCP Benchmark — Instance I10')
    ...
    parser.add_argument('--matrix', type=str,
                        default='mcp_agent_data_16_I10_cost_matrix.csv')
    args = parser.parse_args()

    p_values = list(range(1, args.p_max + 1))

    print(f"\n{'='*60}")
    print(f"QAOA n=16 MCP Agent Coordination Benchmark — INSTANCE I10")
    ...
    out_json = f'{args.output}/results_I10_n16_{args.mode}.json'
