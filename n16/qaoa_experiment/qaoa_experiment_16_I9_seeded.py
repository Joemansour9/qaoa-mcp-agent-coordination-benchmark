"""
qaoa_experiment_16_I9.py
=========================
QAOA Benchmarking — Instance I9 (topology-shifted, patent case P202602120)
...
I9 alone — a single instance measured from a genuinely different task
mix (see mcp_data_generator_16_I9.py), intended as a WebSearch-hub
topology (design partially realized: strong edge-set divergence from
I1, Jaccard=0.286, but ~45% of edges are spoke-to-spoke rather than
routing through WebSearch as designed).
...
Usage:
  ...
  python qaoa_experiment_16_I9.py --mode simulate --p_max 4 \\
      --matrix mcp_agent_data_16_I9_cost_matrix.csv

  # Phase 2 (costs QPU time) — hardware run once warm-start params are ready.
  # NOTE: for the 7m53s budget, run p=2 ONLY first:
  python qaoa_experiment_16_I9.py --mode hardware --token YOUR_TOKEN \\
      --p_max 2 --matrix mcp_agent_data_16_I9_cost_matrix.csv \\
      --sim_results results_I9_n16_simulate.json

  # If quota remains after Run 1, p=4 stretch run:
  python qaoa_experiment_16_I9.py --mode hardware --token YOUR_TOKEN \\
      --p_max 4 --p_min 4 --matrix mcp_agent_data_16_I9_cost_matrix.csv \\
      --sim_results results_I9_n16_simulate.json
"""
...
# Packaged copy: default matrix/output paths below are resolved relative
# to this script's own location (n16/qaoa_experiment/), not the working
# directory, so this runs correctly from anywhere inside the package.
...
def build_instances(W):
    ...
    genuinely different task mix (see mcp_data_generator_16_I9.py),
    ...
    """
    return [(W.copy(), 'I9_websearch_hub_attempt')]
...
def run_experiment(mode='simulate', ibm_token=None,
                   ...
                   matrix_path='mcp_agent_data_16_I9_cost_matrix.csv',
                   max_iter=200, hw_depths=None, sim_results_path=None,
                   hardware_name='ibm_marrakesh'):

    W_full    = load_matrix(matrix_path)
    instances = build_instances(W_full)   # single-element list for I9
...
def plot_results(results, p_values=[1, 2], output_dir='.',
    ...
    short_names = ['I9: WebSearch-hub-attempt']
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}/fig_I9_n16_approx_ratio.png',
                bbox_inches='tight', dpi=300)
    plt.close()
    print("Saved: fig_I9_n16_approx_ratio.png")
    ...
    if has_sim and has_hw:
        ...
        plt.tight_layout()
        plt.savefig(f'{output_dir}/fig_I9_n16_sim_vs_hw.png',
                    bbox_inches='tight', dpi=300)
        plt.close()
        print("Saved: fig_I9_n16_sim_vs_hw.png")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='QAOA n=16 MCP Benchmark — Instance I9')
    ...
    parser.add_argument('--matrix', type=str,
                        default=os.path.join(SCRIPT_DIR, '..', 'cost_matrices',
                                              'mcp_agent_data_16_I9_cost_matrix.csv'))
    args = parser.parse_args()

    p_values = list(range(1, args.p_max + 1))

    print(f"\n{'='*60}")
    print(f"QAOA n=16 MCP Agent Coordination Benchmark — INSTANCE I9")
    ...
    out_json = f'{args.output}/results_I9_n16_{args.mode}.json'
    ...
    if args.mode == 'hardware' and os.path.exists(out_json):
        ...
        # Normalize all keys to strings (JSON round-trips keys as
        # strings; in-memory dict uses int keys from the p_values loop).
