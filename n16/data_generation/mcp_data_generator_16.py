"""
MCP Agent Interaction Data Generator — 16 Node Version
=======================================================
Generates 16x16 cost matrix for n=16 QAOA benchmarking.
...
TASKS = {
    "simple": [
        ...
        "Query the users database for active accounts and schedule a calendar meeting to review the results",
        ...
        "Query the database for recent transactions, run code to calculate totals, and insert the summary into the analytics table",
    ...
    "medium": [
        "Research quantum computing frameworks online, run code to benchmark them, store results in the database, generate an analysis report, and email the summary to the team with a calendar invite for follow-up",
        "Read inbox for customer feedback, embed feedback text for vector search, query similar past issues from the database, run sentiment analysis code, generate a report, and schedule a review meeting",
        "Fetch competitor pricing data from the web, run code to compare with our database pricing, insert findings into the strategy table, generate a report, and email stakeholders with meeting invite",
        "Search for security vulnerabilities, read current policy files, embed policy text, search vector index for related incidents, query the patches database, run analysis code, and write updated advisory",
        "Read sales data files, query targets from database, run code to calculate variance, embed the findings, search similar historical patterns in vector index, generate executive report, and email to leadership",
        "Search for ML best practices, fetch paper URLs, run code to extract key metrics, store in database, embed findings for future search, generate analysis report, and schedule team review",
        "Read inbox for client requests, embed request text, search vector index for similar past projects, query the CRM database, create calendar events for client calls, and generate a proposal report",
        "Fetch API documentation from web, run code to test endpoints, store test results in database, generate technical report, email developer team, and schedule sprint planning meeting",
        "Query the experiments database, run code to compare model performance, embed results for search, generate benchmarking report, email ML team, and create calendar milestone events",
        "Read configuration files, run code to validate settings, search vector index for known issues, query incidents database, generate audit report, email security team, and schedule remediation meeting",
    ],
    "complex": [
        "Conduct full market analysis: search web for trends, fetch 3 competitor URLs, run code to parse pricing data, embed market data for vector search, query CRM database for customer segments, run correlation analysis, insert findings into strategy database, write detailed report to market_analysis.md, generate executive summary, email to leadership team, and schedule quarterly review meeting",
        "Perform complete system audit: read all config files, search for known CVEs online, fetch vulnerability details, run code to test system exposure, embed audit findings, search vector index for similar past incidents, query incidents and patches databases, run statistical analysis, insert results into compliance database, write findings to audit_report.txt, generate executive security report, email CISO, and schedule remediation planning meeting",
        "Build AI pipeline report: read model config files, search arXiv for latest papers, fetch paper abstracts, run code to extract benchmarks, embed abstracts for semantic search, search vector index for related work, query experiments and metrics databases, run comparative analysis, insert evaluation results into ML tracking database, write technical findings to ml_report.md, generate stakeholder summary, email ML team and leadership, and create calendar events for model review sessions",
        "Execute full customer intelligence workflow: read inbox for customer emails, embed email content for vector analysis, search vector index for customer profiles, query CRM and transactions databases, run code to calculate customer lifetime value, run trend analysis on purchase patterns, insert insights into strategy database, write findings to customer_intel.md, generate executive report, email sales team, and schedule quarterly business review meeting",
        "Perform DevOps pipeline audit: read deployment config files, search web for best practices, fetch documentation URLs, run code to test deployment scripts, embed script content for search, search vector index for known issues, query incidents and deployments databases, run statistical analysis on deployment success rates, insert audit results into compliance database, write technical report to devops_audit.md, generate executive summary, email DevOps and engineering leadership, and create calendar events for improvement sprints",
    ]
}
...
def run_experiment(n_tasks: int, output_path: str):
    ...
    print(f"\n{'='*60}")
    print(f"MCP 16-Node Agent Data Generation")
    ...
    for complexity in ['simple', 'medium', 'complex']:
        ...
        for i in range(per_complexity):
            ...
            task_id += 1
            print(f"  [{task_id:3d}/{n_tasks}] {task[:60]}...",
                  end=' ', flush=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=str, default='mcp_agent_data_16.csv')
